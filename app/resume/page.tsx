...
export default function Resume() {
  const { about, experience, skills } = data.resume;
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{
            ...
            <TabsContent value="about" className="w-full">
              <TabsContentWrapper {...about}>
                <div className="grid grid-cols-1 sm:grid-cols-2 mdl:grid-cols-1 lg:grid-cols-2 gap-x-2 gap-y-4 mx-auto mdl:mx-0 max-w-2xl">
                  {about.info.map((elm) => {
                    return (
                      <div key={uuidv4()} className="flex justify-center sm:justify-start gap-1">
              ...
              <TabsContentWrapper {...experience}>
                <ul className="flex">
                  {experience.items.map((elm, index) => {
                    return (
                      <div key={uuidv4()} className="w-full max-w-96 mx-auto mdl:mx-0">
...
// {typeof elm.fieldValue === "string" ? (
//   <p className="">
//     {elm.fieldValue}
//   </p>
// ) : (
//   <ul className="">
//     {elm.fieldValue.map(val => {
//       return (
